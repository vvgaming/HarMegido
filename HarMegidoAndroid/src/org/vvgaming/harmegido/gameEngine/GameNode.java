...
public abstract class GameNode {
	...
	public void kill() {
		murdered = true;
		for (final Entry<Integer, List<GameNode>> entry : nodesPerLayer
				.entrySet()) {
	...
	protected final void realUpdate(final long delta) {
		...
		for (Tuple2<GameNode, Integer> newNode : newNodes) {
			realAddSubNode(newNode.getVal1(), Option.from(newNode.getVal2())
					.getOrElse(0));
		}
		newNodes.clear();
		...
		for (final Entry<Integer, List<GameNode>> entry : nodesPerLayer
				.entrySet()) {
			// cada nó da camada
			for (final Iterator<GameNode> iterator = entry.getValue()
	...
	protected final void realRender(final Canvas canvas) {
		...
		for (final Entry<Integer, List<GameNode>> entry : nodesPerLayer
			...
			final List<GameNode> curLayerList = new ArrayList<GameNode>(
					entry.getValue());
	...
	protected final boolean onRealTouch(final MotionEvent event) {
		...
		final ArrayList<Entry<Integer, List<GameNode>>> entries = new ArrayList<Entry<Integer, List<GameNode>>>(
	 ...
	 * {@link GameNode#render(Canvas)} também. <br/>
	 * Na verdade esse método não adiciona na hora. Ele marca para adicionar o
	 * nó, no início da próxima passada.
	 * 
	 * @param node
	...
	/**
	 * Método interno que faz o que o {@link GameNode#addSubNode(GameNode, int)}
	 * não faz. <br/>
	 * Esse aqui adiciona de verdade, é usado internamente para efetivar a
	 * adição
	 * 
	 * @param node
	 *            o nó a ser adicionado

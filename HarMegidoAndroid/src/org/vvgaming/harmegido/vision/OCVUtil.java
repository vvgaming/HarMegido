...
public class OCVUtil
{
	...
	/**
	 * Converte um {@link OpenCVMatWrapper} em Mat da OpenCV, descomprimindo o conteúdo, deixando-o pronto para uso
	 * 
	 * @param matWrapper
	 * @return
	 */
	public Mat toMat(final OpenCVMatWrapper matWrapper)
	{
		try
		{
			...
			// comprimidos
			final byte[] compressedBytes = Base64.decode(matWrapper.getContent(), Base64.DEFAULT);
			// descomprime os bytes para preparar para utilização
			final byte[] uncompressedBytes = CompressionUtils.decompress(compressedBytes);
	...
	/**
	 * Converte uma Mat da OpenCV em {@link OpenCVMatWrapper}, comprimindo seu conteúdo para agilizar no transporte
	 * 
	 * @param mat
	 * @return
	 */
	public OpenCVMatWrapper toOpenCVMatWrapper(final Mat mat)
	{
		try
		{
			...
			final byte[] compressedBytes = CompressionUtils.compress(uncompressedBytes);
			// agora mudar pra base64 para facilitar no transporte, na rede
			final String content = Base64.encodeToString(compressedBytes, Base64.DEFAULT);
			// por fim, gera o wrapper
			return OpenCVMatWrapper.from(content, mat.rows(), mat.cols(), mat.type());
		}
